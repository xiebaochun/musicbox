...
p.init = function(name) {
	...
	async.series({
		...
		lyrc.listLrcContent(results.lrcContent);
	});
}
...
p.listLrcContent = function(content) {
	...
	$.each(lines, function(index, item) {
		...
		var o_li = {
			...
			lyric: _lyric.replace(/\[|\]/g, '')
		}
		//o_li.lyric = o_li.lyric.replace();
...
p.setItemColorByTime = function(time) {
	$.each(this._lrclist, function(index, item) {
		if (item.time.indexOf(time) > -1) {
			...
			var current_phase = $('.lyric-cur').text();
			
			if(current_phase){
				$('.current-phase').text($('.lyric-cur').text());
			}	
		}
	})
